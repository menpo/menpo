...
def test_scale():
  ...
  assert(_mag_diff(proc.scalerotation_matrices[0], 2*np.eye(2)) < 0.00000001)
...
def test_translate():
  ...
  proc = Procrustes([source],target=target)
  proc.general_alignment()
  assert(_mag_diff(proc.translation_vectors[0], -1.0*np.ones(2))) < 0.00000001)
...
def test_ops():
  ...
  for src in sources:
    ...
    assert(_mag_diff(new_source, proc.aligned_version_of_source(src)) < 0.00000001)
