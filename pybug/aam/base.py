...
class Fitter(object):
    ...
    @abc.abstractmethod
    def _prepare_image(self, image, **kwargs):
        pass

    @abc.abstractmethod
    def fit(self, image_list, initial_target, gt_target_list=None, **kwargs):
        r"""
    ...
    def _fit_image(self, image, initial_target, runs=5, max_iters=20,
                   gt_target=None, verbose=True, view=False):
        ...
        for j in range(runs):
            ...
            basic_fittings = self.fit(image_list, initial_target_list[0],
                                      max_iters=max_iters,
                                      gt_target_list=gt_target_list)
            fitting = self._build_fitting(image, basic_fittings,
                                          affine_correction,
            ...
            if verbose:
                ...
                if gt_target:
                    fitting.print_error
            if view:
                fitting.view_final_fitting()

            fittings.append(fitting)

        return fittings

    def fit_landmarked_images(self, **kwargs):
        pass


class AAMFitter(AAM, Fitter):
    ...
    def initialize(self, lk_algorithm=AlternatingInverseCompositional,
    ...
    def fit(self, image_list, initial_target, gt_target_list=None,
        ...
        lk_fittings = []
        for j, (i, lk) in enumerate(zip(image_list, self._lk_pyramid)):
            ...
            if gt_target_list is not None:
                lk_fitting.set_ground_truth(gt_target_list[j])
    ...
    def lk_fit_landmarked_image(self, image, group=None, label='all',
                                runs=5, noise_std=0.0, rotation=False,
                                max_iters=20, verbose=True, view=False):
        r"""
        Fits the AAM to an image using Lucas-Kanade.
        ...
        aam_fittings = []
        for j in range(runs):
            reference_landmarks = self.shape_model_list[0].mean
            ...
            lk_fittings = self._lk_fit(
                ...
                ground_truth_target_list=ground_truth_pyramid)

            aam_fitting = AAMFitting(image, self, lk_fittings,
                                     affine_correction,
                                     ground_truth=ground_truth)
            aam_fittings.append(aam_fitting)

            if verbose:
                print 'error: {}'.format(aam_fitting.final_error)

        return FittingList(aam_fittings)

    def lk_fit_landmarked_database(self, images, group=None, label='all',
                                   runs=5, noise_std=0.5, rotation=False,
                                   max_iters=20, verbose=True, view=False):
        r"""
...
class FittingList(list, Viewable):
    ...
    def convergence(self):
        ...
        return (np.sum(initial_error > self.final_error) /
                np.product(initial_error.shape))

    @property
    def _error_dist(self):
        final_error = self.final_error
        n_errors = np.product(final_error.shape)
        x_axis = np.arange(0, self._error_stop, self._error_step)
        y_axis = np.array([np.count_nonzero(limit-self._error_step <
                                            final_error[final_error <= limit])
    ...
    def targets(self, as_points=False):
        ...
        for j, f in enumerate(self.basic_fittings):
            ...
            for k, t in enumerate(basic_targets):
                if not self.scaled:
                    t = transform.apply(t)
                basic_targets[k] = self.affine_correction.apply(t)
