...
source = Mock(
    points=source_array, n_dims=source_array.shape[1], n_points=source_array.shape[0]
)
target = Mock(
    points=target_array, n_dims=target_array.shape[1], n_points=target_array.shape[0]
)
target_mismatch = Mock(
    points=target_mismatch_array,
    n_dims=target_mismatch_array.shape[1],
    n_points=target_mismatch_array.shape[0],
)
target_3d = Mock(
    points=target_array_3d,
    n_dims=target_array_3d.shape[1],
