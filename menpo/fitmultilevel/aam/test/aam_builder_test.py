...
@raises(ValueError)
def test_aambuilder_feature_type_exception():
    aam = AAMBuilder(feature_type=['igo', sparse_hog]).build(training_images,
                                                             group='PTS')


@raises(ValueError)
def test_aambuilder_n_levels_exception():
    aam = AAMBuilder(n_levels=0).build(training_images,
                                       group='PTS')


@raises(ValueError)
def test_aambuilder_downscale_exception():
...
@raises(ValueError)
def test_aambuilder_normalization_diagonal_exception():
...
@raises(ValueError)
def test_aambuilder_max_shape_components_exception():
    aam = AAMBuilder(max_shape_components=[1, 0.2, 'a']).build(training_images,
                                                               group='PTS')


@raises(ValueError)
def test_aambuilder_max_appearance_components_exception():
    aam = AAMBuilder(max_appearance_components=[1, 2]).build(training_images,
                                                             group='PTS')


@raises(ValueError)
def test_aambuilder_boundary_exception():
...
@raises(ValueError)
def test_aamfitter_n_shape_exception():
    fitter = LucasKanadeAAMFitter(aam1, n_shape=[3, 6, 'a'])


@raises(ValueError)
def test_aamfitter_n_appearance_exception():
    fitter = LucasKanadeAAMFitter(aam1, n_appearance=[10, 20])


def test_aamfitter_pertrube_shape():
    fitter = LucasKanadeAAMFitter(aam1)
...
def test_aamfitter_obtain_shape_from_bb():
    fitter = LucasKanadeAAMFitter(aam1)
...
def run_test(aam=aam2, algorithm=AlternatingInverseCompositional, im_number=0,
             max_iters=2, initial_error=0.04287, final_error=0.03032,
             error_type='me_norm'):
...
def test_aamfitter_alternating_ic():
    run_test(aam1, AlternatingInverseCompositional, 0, 6, 0.04287, 0.0215,
             'me_norm')


def test_aamfitter_adaptive_ic():
    run_test(aam1, AdaptiveInverseCompositional, 1, 6, 58.78675, 58.98283,
             'me')


def test_aamfitter_simultaneous_ic():
    run_test(aam1, SimultaneousInverseCompositional, 0, 6, 12.66692, 8.7765,
             'rmse')


def test_aamfitter_projectout_ic():
    run_test(aam1, ProjectOutInverseCompositional, 1, 6, 0.64101, 0.63148,
             'me_norm')
