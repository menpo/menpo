...
def test_importing_imageio_RGBA_no_normalise(is_file, mock_image):
    mock_image.return_value = np.zeros([10, 10, 4], dtype=np.uint8)
...
@patch('PIL.Image.open')
@patch('menpo.io.input.base.Path.is_file')
def test_importing_PIL_GIF_normalise(is_file, mock_image):
    mock_image.return_value = PILImage.new('P', (10, 10))
    is_file.return_value = True
...
@patch('PIL.Image.open')
@patch('menpo.io.input.base.Path.is_file')
def test_importing_PIL_GIF_no_normalise(is_file, mock_image):
    mock_image.return_value = PILImage.new('P', (10, 10))
    is_file.return_value = True
...
@patch('menpo.io.input.base.Path.is_file')
@raises(ValueError)
def test_importing_PIL_GIF_non_pallete_exception(is_file, mock_image):
