...
test_lg = mio.import_landmark_file(mio.data_path_to("lenna.ljson"), group="LJSON")
nan_lg = test_lg.copy()
...
def test_export_landmark_ljson_3d(mock_open, exists, json_dump):
    ...
    assert json_dump.call_count == 1
    json_points = np.array(
        json_dump.call_args[0][0]["groups"]["LJSON"]["landmarks"]["points"]
    )
...
def test_export_video_avi_colour(exists, pipe):
    exists.return_value = False
    fake_path = Path("/fake/fake.avi")
    mio.export_video([colour_test_img, colour_test_img], fake_path, extension="avi")
    assert pipe.return_value.stdin.write.call_count == 2
...
def test_export_video_avi_gray_first_mixed(exists, pipe):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        mio.export_video([test_img, colour_test_img], fake_path, extension="avi")
        assert len(w) == 1
    assert pipe.return_value.stdin.write.call_count == 2
    assert "gray8" in pipe.call_args[0][0]
...
def test_export_video_avi_shape_mismatch(exists, pipe):
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        mio.export_video(
            [test_img.resize([150, 100]), test_img], fake_path, extension="avi"
        )
        assert len(w) == 1
    assert "gray8" in pipe.call_args[0][0]
