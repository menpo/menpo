...
class TXTImporter(LandmarkImporter):
    r"""
    Importer for the TXT file format. Assumes version 1 of the format.
    ...
    Note that TXT has a very loose format definition. Here we make the
    assumption (as is common) that TXT landmarks are 1-based. That is,
    landmarks on a 480x480 image are in the range [1-480]. As Menpo is
    consistently 0-based, we *subtract 1* off each landmark value
    automatically.

    If you want to use TXT landmarks that are 0-based, you will have to
    manually add one back on to landmarks post importing.

    Landmark set label: TXT
    ...
    def __init__(self, filepath):
        super(TXTImporter, self).__init__(filepath)
    ...
    def _parse_format(self, asset=None):
        ...
        for line in f:
            if line == '\n' or line == ' \n':
                break
            xpos, ypos, vis = line.split()[0:3]
            xs.append(xpos)
            ys.append(ypos)
            visible.append(vis)
        xs = np.array(xs, dtype=np.float).reshape((-1, 1))
        ys = np.array(ys, dtype=np.float).reshape((-1, 1))
        visible = np.array(visible, dtype=np.int)
        visible = np.array(visible, dtype=np.bool)
        # TXT landmarks are 1-based, need to convert to 0-based (subtract 1)
        points = self._build_points(xs - 1, ys - 1)

        self.group_label = 'TXT'
        self.pointcloud = PointCloud(points)
        self.labels_to_masks = OrderedDict([
            ('all', np.ones(points.shape[0], dtype=np.bool)),
            ('visible', visible)
