...
def rf(X, T):
    ...
    """
    r = ensemble.RandomForestRegressor(n_estimators=20)
    r.fit(X, T)
...
def linear_svr(X, T):
    ...
    """
    regressors = []
    for j, t in enumerate(T.T):
        regressors.append(svm.SVR(kernel='linear').fit(X, t))
        print j

    def linear_svr_predict(x):
        return np.asarray([r.predict(x) for r in regressors])

    return linear_svr_predict
