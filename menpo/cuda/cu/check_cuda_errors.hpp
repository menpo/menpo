...
#define __CLOG__ cudaEvent_t start, stop; float _time_;
#define __START__ cudaEventCreate(&start); cudaEventCreate(&stop); cudaEventRecord(start, 0);
#define __STOP__ cudaEventRecord(stop, 0); cudaEventSynchronize(stop); cudaEventElapsedTime(&_time_, start, stop); cudaEventDestroy(start); cudaEventDestroy(stop); printf("In %s at line %d\t%s\tTOOK: %fms\n", __FILE__, __LINE__, __PRETTY_FUNCTION__, _time_);
