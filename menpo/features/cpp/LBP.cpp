...
LBP::LBP(unsigned int windowHeight, unsigned int windowWidth, unsigned int numberOfChannels,
         ...
         unsigned int mapping_type, unsigned int *uniqueSamples, unsigned int *whichMappingTable,
...
void LBP::apply(double *windowImage, double *descriptorVector) {
    LBPdescriptor(windowImage, this->samples, this->numberOfRadiusSamplesCombinations, this->samples_x_tables, this->samples_y_tables,
                  this->whichMappingTable, this->mapping_tables, this->windowHeight, this->windowWidth, this->numberOfChannels, descriptorVector);
}


void LBPdescriptor(double *inputImage, unsigned int *samples, unsigned int numberOfRadiusSamplesCombinations,
                   double **samples_x_tables, double **samples_y_tables, unsigned int *whichMappingTable,
                   unsigned int **mapping_tables, unsigned int imageHeight, unsigned int imageWidth,
                   unsigned int numberOfChannels, double *descriptorVector) {
    unsigned int i, s, ch;
    int centre_y, centre_x, rx, ry, fx, fy, cx, cy;
    double centre_val, sample_val;
    ...
    for (i=0; i<numberOfRadiusSamplesCombinations; i++) {
        ...
        for (ch=0; ch<numberOfChannels; ch++) {
            ...
            for (s=0; s<samples[i]; s++) {
                ...
                if ( (fabs(samples_x_tables[i][s] - rx) < small_val) && (fabs(samples_y_tables[i][s] - ry) < small_val) )
