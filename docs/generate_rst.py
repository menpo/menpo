...
def write_docs_for_module(module, path, modules_to_skip=None, generate_index=False):
    ...
    for k, v in module.__dict__.iteritems():
        if ismodule(v):
            file_to_doc = docs_for_module(
                k, v, module_name, generate_index=generate_index
            )
...
def docs_for_module(module_name, module, package_name, generate_index=False):
    ...
    for k, v in module.__dict__.iteritems():
        if isclass(v):
            file_to_doc[k] = generate_class_rst(
                module_name, k, module.__name__, package_name
            )
        elif is_func_or_partial(v):
            file_to_doc[k] = generate_function_rst(
                module_name, k, module.__name__, package_name
            )
...
def generate_module_index(module_name, module):
    ...
    toctree = "\n.. toctree::\n  :maxdepth: 2\n\n  "
    items = [
        i for i, v in module.__dict__.items() if isclass(v) or is_func_or_partial(v)
    ]
    return breadcrumb + title + toctree + "\n  ".join(items)


def generate_class_rst(module_name, class_name, module, package_name):
    breadcrumb = ".. _{}-{}-{}:\n\n".format(package_name, module_name, class_name)
...
if __name__ == "__main__":
    ...
    write_docs_for_module(
        menpo, path, generate_index=False, modules_to_skip={"_version"}
    )
